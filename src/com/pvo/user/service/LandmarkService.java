...
public class LandmarkService  extends PVOService<JSONArray>{

	@Override
	public JSONArray executeService(String... params) throws Exception{
		HttpClient httpclient = new DefaultHttpClient();
		HttpPost httppost = new HttpPost(Constant.Landmark.URL);
		List<NameValuePair> nameValuePairs = null;
		if(params.length > 0) {
			nameValuePairs = new ArrayList<NameValuePair>();
			nameValuePairs.add(new BasicNameValuePair(Constant.Landmark.AREA_ID,params[0]));
			httppost.setEntity(new UrlEncodedFormEntity(nameValuePairs));
			String queryString = Utils.getQueryString(nameValuePairs);
			System.out.println("Landmark Query String "+Constant.Landmark.URL+"&"+queryString);
		} /*else {
