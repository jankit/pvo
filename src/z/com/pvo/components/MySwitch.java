...
public class MySwitch extends CompoundButton {
    ...
    public MySwitch(Context context, AttributeSet attrs, int defStyle) {
        ...
        //}
        mTextPaint = new TextPaint(Paint.ANTI_ALIAS_FLAG);//DITHER_FLAG
        Resources res = getResources();
        mTextPaint.density = res.getDisplayMetrics().density;
        mTextPaint.setShadowLayer(0.1f, 1.0f, 1.0f, Color.BLACK);
    ...
    public void setSwitchTextAppearance(Context context, int resid) {
        ...
        if (colors != null) {
            ...
            //mTextColors = ColorStateList.valueOf(Color.RED);
        }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mOrientation == HORIZONTAL) {
        	...
        	if (mPushStyle) {
        		...
        		if (mDrawableOff != null) mDrawableOff.draw(backingLayer);
        		backingLayer.restore();
        		backingLayer.translate(maxTextWidth+ mTrackTextPadding * 2+(maxTextWidth - mOnLayout.getWidth())/2 +mThumbWidth,//+ mThPad.left + mThPad.right,
        				switchVerticalMid - mOnLayout.getHeight() / 2);
        		mOnLayout.draw(backingLayer); 
                if (mDrawableOn != null) mDrawableOn.draw(backingLayer);       		
        		backingLayer.restore();    
